...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the specifications
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    /**
     * render the view for specifications
     *
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'designspecification::specifications/table/ajax';
                $dom_container = '#mod-specifications-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'designspecification::specifications/table/table';
                $dom_container = '#mod-specifications-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'designspecification::specifications/tabswrapper';
            ...
            if ($specifications->currentPage() < $specifications->lastPage()) {
                $url = loadMoreButtonUrl($specifications->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'specifications'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('designspecification::lang.specifications')),
            ];

            //show add specifications button (invoices)
            if ($count > 0) {
                $jsondata['dom_visibility'][] = [
                    'selector' => '#specificationsModalSelectButton',
                    'action' => 'show',
                ];
            }
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($specifications->currentPage() + 1, request('source'));
            $page['loading_target'] = 'mod-specifications-td-container';
            $page['visibility_show_load_more'] = ($specifications->currentPage() < $specifications->lastPage()) ? true : false;
            return view('designspecification::specifications/wrapper', compact('page', 'specifications'))->render();
        }
