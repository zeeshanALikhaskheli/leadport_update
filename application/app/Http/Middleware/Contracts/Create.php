...
class Create {
    ...
    public function handle($request, Closure $next) {
        ...
        //are we loading from the client page
        if (is_numeric(request('contractresource_id')) && request('contractresource_type') == 'client') {
            if (\App\Models\Client::Where('client_id', request('contractresource_id'))->exists()) {
                config(['modal.type' => 'preset-client']);
            }
        }
