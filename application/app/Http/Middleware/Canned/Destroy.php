...
class Destroy {
     ...
     *   <input type="checkbox" name="ids[{{ $canned->canned_id }}]"
     *
    ...
    public function handle($request, Closure $next) {
        ...
        if (is_numeric($request->route('canned'))) {
            $ids[$request->route('canned')] = 'on';
            request()->merge([
                'ids' => $ids,
            ]);
        }
        ...
        //loop through each canned and check permissions
        if (is_array(request('ids'))) {
            ...
            Log::error("no items were sent with this request", ['process' => '[canned][action-bar]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'canned id' => $canned_id ?? '']);
            abort(409);
        }
