...
class LeadPermissions {
    ...
    public function check($action = '', $lead = '') {
        ...
        if (!in_array($action, $this->permissionChecksArray())) {
            Log::info("the requested check is invalid", ['process' => '[permissions][lead]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'check' => $action ?? '']);
            return false;
        }
